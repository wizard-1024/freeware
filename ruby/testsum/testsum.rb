require 'optparse'
require "pathname"
require 'zlib'
require 'openssl'
require 'digest'
require 'digest/sha2'

puts "Build checksums per any file, version 1.0"
puts "Copyright (C) 2022 Dmitry Stefankov. All Rights Reserved."

options = {}
OptionParser.new do |opt|
  opt.on('-i', '--in_dirname FIRSTNAME') { |o| options[:in_dirname] = o }
#  opt.on( '-h', '--help', 'Display this screen' ) { puts opt }
end.parse!

puts options

in_dirname = options[:in_dirname]
puts in_dirname

def procdir(dirname)
  data = ''
  Dir.foreach(dirname) do |dir|
    dirpath = dirname + '/' + dir
    if File.directory?(dirpath) then
      if dir != '.' && dir != '..' then
        #puts "DIRECTORY: #{dirpath}" ; 
        data += procdir(dirpath)
      end
    else
      data += dirpath
    end
  end
  return data
end


def rec_path(path, file= false, dir= false)
  if dir then puts path end
  path.children.collect do |child|
    if file and child.file?
      child
      filesize = File.size(child)
      if (Integer(filesize) < 8*1024*1024) then
        File.open(child, 'rb') do |f|
          in_data = f.read
          f.close
          #puts "filename=",child,"\""
          $stdout.write("filename=\"")
          $stdout.write(child)
          $stdout.write("\"")
          $stdout.write("  ")
          $stdout.write("crc32=\"")
          printf "0x%08x", Zlib.crc32(in_data)
          $stdout.write("")
          $stdout.write("\"")
          s_md5 = Digest::MD5.hexdigest(in_data)
          $stdout.write("  ")
          $stdout.write("md5=\"")
          $stdout.write(s_md5)
          $stdout.write("\"")
          s_sha1 = OpenSSL::Digest::SHA1.hexdigest(in_data)
          $stdout.write("  ")
          $stdout.write("sha1=\"")
          $stdout.write(s_sha1)
          $stdout.write("\"")
          s_sha2_256 = Digest::SHA256.hexdigest(in_data)
          $stdout.write("  ")
          $stdout.write("sha2(256)=\"")
          $stdout.write(s_sha2_256)
          $stdout.write("\"")
          $stdout.write("\n")
        end
      end
    elsif child.directory?
      rec_path(child, file, dir) + [child]
    end
  end.select { |x| x }.flatten(1)
end

# only directories
###rec_path(Pathname.new(in_dirname), false, true)
# directories and normal files
#rec_path(Pathname.new(in_dirname), true, true)
rec_path(Pathname.new(in_dirname), true, false)



#procdir(in_dirname)

# ruby.exe options_flags.rb -h
# ruby.exe file2bin.rb -i test.in -o test.out
