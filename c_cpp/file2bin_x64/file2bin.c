...
void usage( void )
{
  ...
  fprintf( stderr, "Copyright (C) 2006,2016 Dmitry Stefankov. All Rights Reserved.\n" );
...
/* Process command line  */  
  ...
  while( (op = getopt(argc,argv,"fi:l:o:s:vh")) != -1)
    switch(op) {
      ...
      case 'l':
               //i_off = atol(optarg);
               //i_off = strtoull(optarg,NULL,0);
               i_off = _strtoui64(optarg,NULL,0);
               break;
      case 's':
               //i_siz = atol(optarg);
               //i_siz = strtoull(optarg,NULL,0);
               i_siz = _strtoui64(optarg,NULL,0);
               break;
      case 'f':
               force_ask = 1;
               break;
  ...
  if (verbose) {
    printf( "i_fname:'%s'\n", i_fname );
    printf( "o_fname: '%s'\n", o_fname );
    printf( "i_siz: '%llu' (0x%016X)\n", i_siz, i_siz );
    printf( "i_off: '%llu' (0x%016X)\n", i_off, i_off );
  }
  ...
  res = _fseeki64( ifp, i_off, SEEK_SET );
  if (res < 0) {
