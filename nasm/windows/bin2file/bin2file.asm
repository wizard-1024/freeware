...
; extract output offset number
    xor     rcx,rcx
    mov     rdi, OutOffsetBuf
NextSym6:
    mov     al, byte [rbx]
    or      al, al
;   jz      Finished
...
;  DWORD  dwMoveMethod
;);   
	mov 	rcx, [fOutHandle]	        ;handle
	mov 	rdx, [OutOffsetVal]		;low bits of position
	mov 	r8, 0         			;no high order bits in position
	mov 	r9, 0     		        ;start from beginning = FILE_BEGIN
	call	SetFilePointer
        cmp     rax, INVALID_SET_FILE_POINTER
...
crlf_buf           db    13,10,0
crlf_len           dq    2
NtlpBuffer         db    'Put binary portion to file, version 1.0', 13,10, 
...
Msg_06_Len               dq  $ - Msg_02_CannotSeekFile - 1
InFileNameLen      dq    0
OutFileNameLen     dq    0
InOffsetLen        dq    0
OutOffsetLen       dq    0
